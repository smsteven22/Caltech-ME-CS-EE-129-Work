    ...
    def dijkstras(self, goal):
        ...
        while len(onDeck) > 0:
            ...
            for heading in range(8):
                # Check if the given heading is connected to the currentintersection
                if current.streets[heading] == Status.CONNECTED or current.streets[heading] == Status.UNEXPLORED:
                    dx = heading_to_dx[heading]
                    dy = heading_to_dy[heading]
